        ...
        <div className={"box"}>
            {Object.keys(val).map((key, eleI) =>
                     <div key={eleI}>
                            ...
                            <span >
                                {
                                    (!Object.values(val)[eleI])&&<span className={"empty content"}>null</span>
                                }
                                {
                                    (Object.values(val)[eleI]&&Object.values(val)[eleI].length===0)&&<span className={"empty content"}>[]</span>
                                }
                    {
                        ...
                        <>
                            ...
                            <span className={"boolean content"}> {val.sortedList ? "true" : "false"}
                                ...
                                {(key === "rows" && isTableType(val))
                                    ? <ObjectArray objects={val.tableRows}/> : ""
                                }
                                {(key === "paragraphs" || key === "tableTitles") && (isTableType(val) || isListType(val))
